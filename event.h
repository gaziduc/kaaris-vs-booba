...
#define EVENT_H
    ...
    #define KEY_UP_EDITOR       in->key[SDL_SCANCODE_UP] || in->controller[0].hat[0] == SDL_HAT_UP || in->controller[1].hat[0] == SDL_HAT_UP || (in->controller[0].axes[1] < -DEAD_ZONE && in->controller[0].is_axes_centered[1]) || (in->controller[1].axes[1] < -DEAD_ZONE && in->controller[1].is_axes_centered[1])
    #define KEY_DOWN_EDITOR     in->key[SDL_SCANCODE_DOWN] || in->controller[0].hat[0] == SDL_HAT_DOWN || in->controller[1].hat[0] == SDL_HAT_DOWN || (in->controller[0].axes[1] > DEAD_ZONE && in->controller[0].is_axes_centered[1]) || (in->controller[1].axes[1] > DEAD_ZONE && in->controller[1].is_axes_centered[1])
    #define KEY_LEFT_EDITOR     in->key[SDL_SCANCODE_LEFT] || in->controller[0].hat[0] == SDL_HAT_LEFT || in->controller[1].hat[0] == SDL_HAT_LEFT || (in->controller[0].axes[0] < -DEAD_ZONE && in->controller[0].is_axes_centered[0]) || (in->controller[1].axes[0] < -DEAD_ZONE && in->controller[1].is_axes_centered[0])
    #define KEY_RIGHT_EDITOR    in->key[SDL_SCANCODE_RIGHT] || in->controller[0].hat[0] == SDL_HAT_RIGHT || in->controller[1].hat[0] == SDL_HAT_RIGHT || (in->controller[0].axes[0] > DEAD_ZONE && in->controller[0].is_axes_centered[0]) || (in->controller[1].axes[0] > DEAD_ZONE && in->controller[1].is_axes_centered[0])
